...
export function tarifPourHeure(heure) {
  ...
  if (!grille.length) {
    return {
      trouve: false,
      message:
        'ABSENT DU CONTRAT : le tableau des tarifs (§3.4) est introuvable dans docs/CAHIER-API.md. Ne pas inventer de prix : demander a Junior.',
    }
  }
  ...
  if (!ligne) {
    ...
    return {
      ...
      message:
        ...
        `Ne pas extrapoler : demander a Junior.`,
    }
  }
  return {
    trouve: true,
  ...
  if (manquants.length) {
    return {
      erreur:
        ...
        `Ne pas supposer une valeur : demander a Junior.`,
    }
  }
  ...
  return {
    ...
    avertissement: divergence
      ? `ARBITRAGE NON TRANCHE (spec 06 §11 Q7) : le cahier dit « moins de 24 h avant le creneau » ` +
        ...
        `Les deux sont donnees : ne pas en choisir une sans l'accord de Junior.`
      : null,
    sources: [
  ...
  if (!dansEnum && !dansTable) {
    return {
      ...
      message:
        ...
        `Ne pas inventer : demander a Junior.`,
    }
  }
    ...
    return {
      ...
      citation_introuvable: cit
        ...
        : `La phrase attendue (§${a.section}, motif "${a.motif}") n'est plus dans ${cahier.FICHIER}. ` +
          `Le cahier a change : cette transition est a revalider avec Junior avant de coder.`,
    }
  })
  ...
  if (s === 'held') {
    ...
    lacunes.push({
      ...
      detail:
        ...
        'Ne pas inventer `cancelled` : demander a Junior quel statut porter.',
      citation: cit,
    })
  }
