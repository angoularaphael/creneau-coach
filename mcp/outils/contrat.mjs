...
const listerEndpoints = {
  ...
  config: {
    ...
    inputSchema: {
      ...
      lot: z.string().optional().describe('Filtre par lot : A (Raphael), B (Brad), C (Eddy)'),
  ...
  async handler({ tag, lot, prefixe }) {
    ...
    return texte(
      `${filtrees.length} operation(s) sur ${oas.operations().length} declarees.\n\n${corps}\n\n` +
        ...
        `(« 6. Reservations — Eddy » → lot C). Un « — » signifie que le cahier ne documente pas ` +
    ...
    const blocs = regles.map((x) => {
      ...
      return (
        ...
        `\n  Phrase du cahier §${x.section} : ${
          cit ? `« ${cit.texte} » (ligne ${cit.ligne})` : `INTROUVABLE — le motif "${x.motif}" n'est plus dans ${cahier.FICHIER} §${x.section}. Le cahier a change : revalider avec Eddy.`
        }`
      )
    })
